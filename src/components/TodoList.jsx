        ...
        todosShow.length > 0 ?
          <ul className='todo-list'>
            {todosShow.map((item) => {
              ...
              // 이 item이 수정중 화면인지 출력화면인지 edit이라는 이름으로 저장해둠
              ...
              const editView = (
                <li key={item.id} id={item.id}>
                  ...
                  <button className="cancel-btn" onClick={() => { onEdit(item.id, editText) }}>취소</button>
                </li>
              )
              ...
              //수정중이면 editview 보여주고 아니면 normalView로 보여줌
            })}
          </ul>
          :
          <p className='empty-list'>아직 할 일이 없어요</p>
      }
    </div>
  )
}
